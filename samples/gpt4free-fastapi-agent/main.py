...
This exposes just enough of the OpenAI API for code agents such as
Codex CLI, Aider, Continue, etc. to talk to it:

    GET  /v1/models
    POST /v1/chat/completions   (Chat Completions wire format)
    POST /v1/responses          (Responses wire format, e.g. Codex CLI)

Both endpoints support streaming and non-streaming, and both are backed
by the same free, no-auth g4f provider (Pollinations) by default.
...
See README.md in this directory for setup and usage details, including
Codex CLI's `wire_api = "responses"` custom model provider config.
"""
...
# Backend model/provider actually used for every request, regardless of
# what model name the caller sends (Codex, for example, will send
# "gpt-4o" — we always serve it from the free no-auth backend below).
